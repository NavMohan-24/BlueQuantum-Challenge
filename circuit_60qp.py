...
qc = qiskit.qasm2.load('circuit_3_60q.qasm', include_path=('.',), include_input_directory='append', custom_instructions=qiskit.qasm2.LEGACY_CUSTOM_INSTRUCTIONS, custom_classical=(), strict=False)
...
ncuts = 6
subcircuits, subobservables, bases = cutting_circuits(qc, ncuts)
num_samples = 10

print('Number_of_cuts:',ncuts,'Number_of_samples:',num_samples)
...
def run_subexperiment(label, sub_experiment):
    with Batch(backend=backend) as batch:
        ...
        job = sampler.run(sub_experiment, shots=2**((qc.num_qubits//ncuts)+2)).result()
        return label, job
...
results = knitting_results(job_results)
print('State of Maximum Probability:', results )
